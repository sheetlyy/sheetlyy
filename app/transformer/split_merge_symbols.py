...
from app.transformer.circle_of_fifths import (
    KeyTransformation,
    NoKeyTransformation,
    key_signature_to_circle_of_fifth,
)
from app.transformer.configs import default_config
from app.utils.constants import TRIPLET_SYMBOL
...
class SymbolMerger:
    ...
    def _append_symbol(self, symbol: str) -> None:
        if self.next_symbol_is_chord:
            if len(self.merge) == 0:
                logger.warning("Warning: Unexpected chord symbol")
                return
    ...
    def add_symbol(self, predrhythm: str, predpitch: str, predlift: str) -> bool:
        ...
        if predrhythm == "|":
            ...
            if self.last_clef and self.last_clef != predrhythm:
                logger.warning("Warning: Two clefs in a staff")
                return True
    ...
    def complete(self) -> str:
        merged = [
            str.join("|", self._clean_and_sort_chord(symbols)) for symbols in self.merge
        ]
...
def _translate_duration(duration: str) -> str:
    ...
    duration = duration.replace(TRIPLET_SYMBOL, "")
    return duration
...
def _symbol_to_rhythm(symbol: str) -> str:  # noqa: PLR0911
    ...
    symbol = symbol.replace(TRIPLET_SYMBOL, "")
...
def pitch_name_to_sortable(pitch_or_rest_name: str) -> int:
    if pitch_or_rest_name.startswith("rest"):
        ...
        if pitch_or_rest_name in default_config.rhythm_vocab:
            ...
            logger.warning("Warning: rest not in rhythm_vocab", pitch_or_rest_name)
            return 1000
