...
from worker.utils.constants import DURATION_OF_QUARTER, TRIPLET_SYMBOL
...
class TrOMRParser:
    ...
    def parse_clef(self, clef: str) -> ResultClef:
        ...
        clef_type = ClefType(clef_type_str[0])
    ...
    def parse_key_signature(self, key_signature: str, clef: ResultClef) -> None:
        ...
        if signature_name in key_signature_mapping:
            ...
            logger.warning("WARNING: Unrecognized key signature: " + signature_name)
    ...
    def parse_duration_name(self, duration_name: str) -> int:
        duration_mapping = {
            "whole": DURATION_OF_QUARTER * 4,
            "half": DURATION_OF_QUARTER * 2,
            "quarter": DURATION_OF_QUARTER,
            "eighth": DURATION_OF_QUARTER // 2,
            "sixteenth": DURATION_OF_QUARTER // 4,
            "thirty_second": DURATION_OF_QUARTER // 8,
        }
        return duration_mapping.get(duration_name, DURATION_OF_QUARTER // 16)

    def parse_duration(self, duration: str) -> ResultDuration:
        has_dot = duration.endswith(".")
        is_triplet = duration.endswith(TRIPLET_SYMBOL)
    ...
    def parse_note(self, note: str) -> ResultNote:
        try:
            ...
            return ResultNote(
                ResultPitch(note_name, octave, alter), self.parse_duration(duration)
            )
        except Exception:
            logger.error("Failed to parse note: " + note)
            return ResultNote(
                ResultPitch("C", 4, 0), ResultDuration(DURATION_OF_QUARTER)
            )
    ...
    def parse_notes(self, notes: str) -> ResultChord | None:
        ...
        rest_parts = [
            rest_part
            for rest_part in note_or_rest_parts
            if rest_part.startswith("rest")
        ]
        note_parts = [
            note_part
            for note_part in note_or_rest_parts
            if note_part.startswith("note")
        ]
        result_notes = [self.parse_note(note_part) for note_part in note_parts]
        min_duration = self.get_min_duration(result_notes)
    ...
    def get_min_duration(self, notes: list[ResultNote]) -> ResultDuration:
        if len(notes) == 0:
            return ResultDuration(DURATION_OF_QUARTER)
    ...
    def parse_tr_omr_output(self, output: str) -> ResultStaff:  # noqa: C901
        ...
        for part in parts:
            if part == "barline":
                ...
                logger.info("Skipping over multirest")
