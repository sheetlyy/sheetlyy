...
from worker.transformer.configs import Config
from worker.transformer.split_merge_symbols import SymbolMerger
...
class ScoreTransformerWrapper(nn.Module):
    ...
    def calculate_center_of_attention(self, intermediates: Any) -> tuple[float, float]:
...
class ScoreDecoder(nn.Module):
        ...
        for _position_in_seq in range(seq_len):
            ...
            while retry and attempt < max_attempts:
                ...
                retry = merger.add_symbol(
                    rhythm_token[0], pitch_token[0], lift_token[0]
                )
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        loss_consist = self.calConsistencyLoss(rhythmsp, pitchsp, liftsp, notesp, mask)  # type: ignore
        loss_rhythm = self.masked_logits_cross_entropy(rhythmsp, rhythmso, mask)  # type: ignore
        loss_pitch = self.masked_logits_cross_entropy(pitchsp, pitchso, mask)  # type: ignore
        loss_lift = self.masked_logits_cross_entropy(liftsp, liftso, mask)  # type: ignore
        loss_note = self.masked_logits_cross_entropy(notesp, noteso, mask)  # type: ignore
