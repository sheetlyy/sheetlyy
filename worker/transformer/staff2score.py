...
from worker.transformer.tromr_arch import TrOMR
...
class Staff2Score:
    def __init__(self, config: Config, keep_all_symbols_in_chord: bool = False) -> None:
        ...
        checkpoint_file_path = str(config.filepaths.checkpoint)
        ...
        if ".safetensors" in checkpoint_file_path:
            ...
            self.model.load_state_dict(
                torch.load(checkpoint_file_path, map_location=torch.device("cpu")),
                strict=False,
            )
        ...
        if not os.path.exists(config.filepaths.rhythmtokenizer):
            raise RuntimeError(
                ...
                + str(config.filepaths.rhythmtokenizer)
            )

    def predict(self, image: NDArray) -> list[str]:
